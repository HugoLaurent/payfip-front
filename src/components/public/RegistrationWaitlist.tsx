...
// demandé (voir maquette écran E1 mobile). Desktop : même gabarit que les
// autres écrans d'état — détail à gauche, action à droite.
export function RegistrationWaitlist({
  ...
  const dateLine = [formation.eventDate ? formatDateLabel(formation.eventDate) : null, formation.timeLabel]
    .filter(Boolean)
    .join(' · ')
  ...
  return (
    ...
    >
      <div className="flex flex-col items-center gap-4 text-center md:flex-row md:items-start md:gap-5 md:text-left">
        <div
          className="flex h-[78px] w-[78px] shrink-0 items-center justify-center rounded-full bg-[oklch(0.95_0.02_265)] text-[30px] font-bold text-aregie-deep"
      ...
      <div className="flex flex-col gap-[13px] md:grid md:grid-cols-[1fr_320px] md:items-start">
        <div className="flex flex-col gap-[13px]">
          <div className="squircle flex w-full flex-col gap-[13px] rounded-[18px] bg-date-tint px-[18px] py-[17px] text-left md:rounded-[22px]">
          ...
          <div className="squircle w-full rounded-[18px] border-[1.5px] border-hairline px-[17px] py-[15px] text-left md:rounded-[22px]">
        ...
        <div className="flex flex-col gap-[10px]">
          {canConfirm && (
            <div className="squircle flex w-full flex-col gap-2 rounded-[18px] border-2 border-aregie-coral bg-[oklch(0.98_0.012_35)] px-[17px] py-[15px] text-left md:rounded-[22px]">
          ...
          {leaveError && <p className="text-center text-sm text-red-600">{leaveError}</p>}
        </div>
      </div>
    </motion.div>
  )
